...
void PocniVrijeme(clock_t& vrijeme) {
    vrijeme = clock();
}

void ZaustaviVrijeme(clock_t vrijeme) {
    clock_t vrijeme2 = clock();
    cout << "Prolazno vrijeme: " << (vrijeme2 - vrijeme) /  (CLOCKS_PER_SEC / 1000.0) << endl;
}

int main() {
    const int BROJ_EL = 1000;
    while(1) {
        Lista<int> *l = nullptr;
        int n;
        clock_t v;
        do {
        cout << "Odaberite tip(0 - Niz, 1 - Jednostruka): ";
        cin >> n;
        } while(n < 0 || n > 1);
        if(n == 0)  l = new NizLista<int>();
        else l = new JednostrukaLista<int>();
        try {
            cout << "Kreiranje " << BROJ_EL << " elemenata (dodavanjem iza). ";
            PocniVrijeme(v);
            for(int i = 0; i < BROJ_EL; i++ )
                l->dodajIza(i);
            ZaustaviVrijeme(v);
        
            cout << "Dodavanje " << BROJ_EL << " elemenata (dodavanjem ispred). ";
            PocniVrijeme(v);
            for(int i = 0; i < BROJ_EL; i++ )
                l->dodajIspred(i + BROJ_EL);
            ZaustaviVrijeme(v);
            
            cout << "Pristup prvom elementu. ";
            l->kraj();
            PocniVrijeme(v);
            l->pocetak();
            ZaustaviVrijeme(v);
            
            cout << "Pristup zadnjem elementu. ";
            PocniVrijeme(v);
            l->pocetak();
            ZaustaviVrijeme(v);
            
            cout << "Brisanje " << BROJ_EL << " elemenata (sa kraja). ";
            PocniVrijeme(v);
            for(int i = 0; i < BROJ_EL - 1; i++ )
                l->obrisi();
            ZaustaviVrijeme(v);
            
            cout << "Brisanje " << BROJ_EL << " elemenata (sa pocetka). ";
            l->pocetak();
            PocniVrijeme(v);
            for(int i = 0; i < BROJ_EL; ++i )
                l->obrisi();
            ...
            cout << "Pristup " << BROJ_EL << ". elementu indeksiranjem (od pocetka). ";
            for(int i = 0; i < 2 * BROJ_EL; ++i )
                l->dodajIza(i);
            PocniVrijeme(v);
            (*l)[BROJ_EL - 1];
            ZaustaviVrijeme(v);
